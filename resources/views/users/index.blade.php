...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        ...
        @endif
        <p><a class ="search" href="{{route('post.search')}}">検索</a></p>
  ...
  　　　           <div class="container px-5 py-24 mx-auto ">
   　　　          <div class="flex items-center lg:w-3/5 mx-auto border-b pb-10 mb-10 border-gray-200 sm:flex-row flex-col">
                ...
                <a class="user" href='/users/{{$post->user_id}}'>User：{{$post->user->name}}</a><br>
                ...
                <div class="flex flex-wrap gap-2">
                 <span class="rounded-full bg-primary-50 py-1 pr-3 text-sm font-semibold text-yellow-600"><a href="/regions/{{$post->region->id}}">{{ $post->region->name}}</a></span>
                  <span class="rounded-full bg-green-50 py-1 text-sm font-semibold text-green-600">@foreach($post->categories as $category)
                  <a  href="/categories/{{$category->id}}">{{ $category->name }}</a>
                     @endforeach</span>
             </div>
                <p class="city leading-relaxed text-base">{{$post->city}}</p> 
