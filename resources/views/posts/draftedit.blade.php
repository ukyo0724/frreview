...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        ...
        <form action="{{route('draftupdate', ['post'=>$post->id])}}" method="POST"  id="form_edit" enctype="multipart/form-data">
            @csrf
        <div class="post-edit-container">
            <div class="mx-auto max-w-md mb-5">
                <div>
                    <label for="title" class="mb-1 mt-10 block text-sm font-medium text-gray-700">タイトル</label>
                    <input type="email" name="post[title]" id="title" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-400 focus:ring focus:ring-primary-200 focus:ring-opacity-50 disabled:cursor-not-allowed disabled:bg-gray-50 disabled:text-gray-500" value="{{$post->title}}" value="{{old('post.city')}}"/>
            ...
            <div class="mx-auto max-w-md mt-5 mb-5">
                <div>
                    ...
                    <input type="email" name="post[city]" id="city" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-400 focus:ring focus:ring-primary-200 focus:ring-opacity-50 disabled:cursor-not-allowed disabled:bg-gray-50 disabled:text-gray-500" value="{{$post->city}}" value="{{old('post.city')}}"/>
            ...
            <div class="flex flex-row items-center max-auto justify-center mt-7">
                <table class="table mt-7">
                    <td class="preview mt-7 w-160 mt-4" id="imgPreviewField"></td>
                </table>
            </div>
        </div>
        ...
        <form action="/posts/image/delete/{{$post->id}}" id="form_image_delete_{{ $post->id }}" method="post">
            <div class="image-index flex flex-row items-center justify-center mt-11">
                @csrf
                @method("DELETE")
            ...
            <div class="flex flex-row items-center justify-center max-auto">
